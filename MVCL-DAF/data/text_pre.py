...
class DataProcessor(object):
    ...
    @classmethod
    def _read_tsv(cls, dataset, input_file, quotechar=None):
...
class DatasetProcessor(DataProcessor):
    ...
    def __init__(self, args):
        ...
        if args.dataset in ['MIntRec']:  
            self.select_id = 3
            self.label_id = 4
        elif args.dataset in ['MELD','MIntRec2']:   
            self.select_id = 2
            self.label_id = 3
    ...
    def get_examples(self, args, data_dir, mode):
        if mode == 'train':
            return self._create_examples(
                self._read_tsv(dataset=args.dataset, input_file=os.path.join(data_dir, "train.tsv")), "train")
        elif mode == 'dev':
            return self._create_examples(
                self._read_tsv(args.dataset,os.path.join(data_dir, "dev.tsv")), "train")
        elif mode == 'test':
            return self._create_examples(
                self._read_tsv(args.dataset,os.path.join(data_dir, "test.tsv")), "test")
        elif mode == 'all':
            return self._create_examples(
                self._read_tsv(args.dataset,os.path.join(data_dir, "all.tsv")), "all")
...
def convert_examples_to_features(args, examples, data_args, tokenizer):
    ...
    max_cons_seq_length = max_seq_length  + label_len
    for (ex_index, example) in enumerate(examples):
        ...
        cons_tokens = ["[CLS]"] + tokens_a + condition + (label_len - len(condition)) * ["MASK"] + ["[SEP]"]

        tokens = ["[CLS]"] + tokens_a + label_len * ["[MASK]"] + ["[SEP]"]
        ...
        condition_idx.append(1 + len(tokens_a))
